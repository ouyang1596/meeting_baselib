...
public class IdentifyNumPage extends FakeActivity implements OnClickListener, TextWatcher {
	...
	public void onTextChanged(CharSequence s, int start, int before, int count) {
		...
		if (s.length() > 0) {
			...
			int resId = getBitmapRes(activity, "rose_blue_radius_bg");
			if (resId > 0) {
		...
		if (type == 1) {
			...
			if (resId > 0) {
				...
				if (resId > 0) {
					...
					resId = getStringRes(activity, "smssdk_make_sure_send_sounds");
					if (resId > 0) {
		...
		runOnUIThread(new Runnable() {
			public void run() {
				...
				if (result == SMSSDK.RESULT_COMPLETE) {
					if (callback != null) {
						callback.afterEvent(SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE, SMSSDK.RESULT_COMPLETE, phone);
					}
					finish();
				} else {
